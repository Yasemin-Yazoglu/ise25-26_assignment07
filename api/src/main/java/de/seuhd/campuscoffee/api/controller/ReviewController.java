...
public class ReviewController extends CrudController<Review, ReviewDto, Long> {
    ...
    public @NonNull ResponseEntity<ReviewDto> update(
            @Parameter(description = "Unique identifier of Review to update", required = true)
            @PathVariable Long id,
            @Parameter(description = "Data of the updated Review", required = true)
            @RequestBody @Valid ReviewDto reviewDto
    ) {
    ...
    public @NonNull ResponseEntity<Void> delete(
            @Parameter(description = "Unique identifier of Review to delete", required = true)
            @PathVariable Long id
    ) {
    ...
    public ResponseEntity<List<ReviewDto>> filter(
            @Parameter(description = "Unique identifier of POS", required = true)
            @RequestParam("pos_id") Long posId,
            @Parameter(description = "approved status", required = true)
    ...
    public ResponseEntity<ReviewDto> approve(
            @Parameter(description = "Unique identifier for review", required = true)
            @PathVariable Long id,
            @Parameter(description = "Unique identifier for user")
            @RequestParam("user_id") Long userId
    ) {
        Review review = reviewService.getById(id);

        return ResponseEntity.ok(
                reviewDtoMapper.fromDomain(reviewService.approve(review, userId))
        );
    }
}
